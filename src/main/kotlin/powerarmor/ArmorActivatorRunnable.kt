...
class ArmorActivatorRunnable : BukkitRunnable() {
	...
	override fun run() {
		...
		getServer().onlinePlayers.forEach { player ->
			if (PowerArmorManager.isWearingPowerArmor(player) && PowerArmorManager.getPowerArmorEnabled(player) && PowerArmorManager.getArmorPower(player) > 0 && PowerArmorManager.getCurrentModuleWeight(player) <= PowerArmorManager.maxModuleWeight) {
